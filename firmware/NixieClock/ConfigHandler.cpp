...
ConfigHandler::ConfigHandler(const char *thingName, const char *wifiInitialApPassword, const char *Version, DNSServer &dnsServer,
        ...
        server(webserver),
        ntpServerParam(IotWebConfParameter("NTP Server", "ntpServerParam", this->ntpServerName, STRING_LEN, "text",
...
String ConfigHandler::GetNTPServerName()
{
    return String(ntpServerName);
}

int ConfigHandler::GetNTPUpdateInterval()
{
    return atol(ntpUpdateRate) * 60;
}

int ConfigHandler::GetTimezone()
{
    return atol(ntpTimeZone);
}

bool ConfigHandler::GetDLS()
{
...
int ConfigHandler::GetLEDBrightness()
{
    return atoi(ledBrightness);
}

bool ConfigHandler::GetAnimateDigits()
{
...
bool ConfigHandler::GetTouchEnabled()
{
...
int ConfigHandler::GetDisplayOnTime()
{
